...
def find_available_time(calendar_service, start_datetime, duration=60):
    ...
    available_start_time = start_time.astimezone(timezone.utc)
    for event in events:
        event_start = datetime.fromisoformat(event['start']['dateTime']).strftime('%Y-%m-%dT%H:%M:%S') + 'Z'
        event_end = datetime.fromisoformat(event['end']['dateTime']).strftime('%Y-%m-%dT%H:%M:%S') + 'Z'
